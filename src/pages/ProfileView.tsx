...
const ProfileView: React.FC = () => {
  ...
  const handleDeleteSuccess = () => {
    // This function is called from the modal on successful deletion
    setIsDeleteModalOpen(false);
    if(setUser) {
        setUser(null); // Clear user from context, which logs them out
    }
  ...
  return (
    <>
        ...
        >
          ...
          <form onSubmit={handleSave}>
            ...
            {/* --- ADD THIS NEW SECTION for the delete button --- */}
              {editMode && (
                <div className="mt-12 pt-6 border-t border-red-500/30 text-center">
                    <h3 className="text-lg font-semibold text-red-600 dark:text-red-400">Danger Zone</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400 my-2 max-w-md mx-auto">
                      Deleting your account is permanent. All your data, including quiz results and reminders, will be removed and cannot be recovered.
                    </p>
                    <button
                      type="button"
                      onClick={() => setIsDeleteModalOpen(true)}
                      className="mt-4 bg-transparent text-red-600 border border-red-600 px-6 py-2 rounded-full font-bold hover:bg-red-600 hover:text-white transition-colors"
                    >
                      Delete My Account
                    </button>
                </div>
      ...
      {/* Add the new modal to the component's return */}
      <DeleteAccountModal 
