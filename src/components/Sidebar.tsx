...
type SidebarProps = {
  user: any;
  activeView: string;
  setActiveView: React.Dispatch<React.SetStateAction<'dashboard' | 'profile' | 'feedback'>>;
  handleLogout: () => void;
};
...
const Sidebar: React.FC<SidebarProps> = ({ user, activeView, setActiveView, handleLogout }) => {
  const sidebarLinks = [
    ...
    { name: 'Profile', icon: <Cog6ToothIcon className="w-6 h-6" />, action: () => setActiveView('profile') },
  ...
  return (
    <aside className="w-64 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm shadow-lg flex-col hidden lg:flex">
      ...
      <div className="p-4 border-b border-gray-200 dark:border-gray-700">
        <button className="w-full flex items-center space-x-4 px-4 py-3 text-gray-600 dark:text-gray-300 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700/50">
        ...
        {sidebarLinks.map((link) => (
          link.path ? (
            <Link
              key={link.name}
              to={link.path}
              className={`flex items-center space-x-4 px-4 py-3 rounded-lg transition-colors ${activeView === link.name.toLowerCase() ? 'bg-ayurGreen/10 text-ayurGreen font-bold dark:bg-gray-700' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700/50'}`}
            >
              {link.icon}
              <span>{link.name}</span>
            </Link>
            ...
            <button
              key={link.name}
              onClick={link.action}
              className={`w-full flex items-center space-x-4 px-4 py-3 rounded-lg transition-colors ${activeView === link.name.toLowerCase() ? 'bg-ayurGreen/10 text-ayurGreen font-bold dark:bg-gray-700' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700/50'}`}
            >
              {link.icon}
              <span>{link.name}</span>
            </button>
